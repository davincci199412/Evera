...
import com.evera.core.worker.Worker;
...
import static com.evera.core.task.TaskStatus.*;
import static com.evera.core.utils.DateTimeUtils.sleep;
...
public class TaskServiceTests {
    ...
    public void shouldNotUpgrade2ReopenedSinceCurrentStatusWrong() {
        ...
        when(everaHubService.canReopen(task.getChainTaskId())).thenReturn(true);
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.reOpen(task.getChainTaskId())).thenReturn(Optional.of(new ChainReceipt()));
    ...
    public void shouldNotUpgrade2ReopenedSinceCantReopenOnChain() {
        ...
        task.setRevealDeadline(new Date(new Date().getTime() - 10));
        when(replicatesService.getNbReplicatesWithCurrentStatus(CHAIN_TASK_ID, ReplicateStatus.REVEALED)).thenReturn(0);
        when(everaHubService.canReopen(task.getChainTaskId())).thenReturn(false);
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.reOpen(task.getChainTaskId())).thenReturn(Optional.of(new ChainReceipt()));
    ...
    public void shouldNotUpgrade2ReopenedSinceNotEnoughtGas() {
        ...
        when(replicatesService.getNbReplicatesWithCurrentStatus(CHAIN_TASK_ID, ReplicateStatus.REVEALED)).thenReturn(0);
        when(everaHubService.canReopen(task.getChainTaskId())).thenReturn(true);
        when(everaHubService.hasEnoughGas()).thenReturn(false);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.reOpen(task.getChainTaskId())).thenReturn(Optional.of(new ChainReceipt()));
    ...
    public void shouldNotUpgrade2ReopenedBut2ReopendedFailedSinceTxFailed() {
        ...
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.reOpen(task.getChainTaskId())).thenReturn(Optional.empty());
    ...
    public void shouldNotUpdateReceived2InitializingSinceNoEnoughGas() {
        ...
        when(taskRepository.findByChainTaskId(CHAIN_TASK_ID)).thenReturn(Optional.of(task));
        when(everaHubService.canInitialize(CHAIN_DEAL_ID, 1)).thenReturn(true);
        when(everaHubService.hasEnoughGas()).thenReturn(false);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.initialize(CHAIN_DEAL_ID, 1)).thenReturn(Optional.of(pair));
        when(everaHubService.getChainTask(CHAIN_TASK_ID)).thenReturn(Optional.of(ChainTask.builder().build()));
    ...
    public void shouldNotUpdateReceived2InitializingSinceCantInitialize() {
        ...
        when(taskRepository.findByChainTaskId(CHAIN_TASK_ID)).thenReturn(Optional.of(task));
        when(everaHubService.canInitialize(CHAIN_DEAL_ID, 1)).thenReturn(false);
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(taskRepository.save(task)).thenReturn(task);
        when(everaHubService.initialize(CHAIN_DEAL_ID, 1)).thenReturn(Optional.of(pair));
        when(everaHubService.getChainTask(CHAIN_TASK_ID)).thenReturn(Optional.of(ChainTask.builder().build()));
    ...
    public void shouldNotUpdateInitializing2InitailizedSinceNoChainTaskReturned() {
        ...
        when(taskRepository.findByChainTaskId(CHAIN_TASK_ID)).thenReturn(Optional.of(task));
        when(everaHubService.initialize(CHAIN_DEAL_ID, 1)).thenReturn(Optional.of(pair));
        when(everaHubService.getChainTask(CHAIN_TASK_ID)).thenReturn(Optional.empty());
    ...
    public void shouldNotUpdateInitializedOrRunning2ContributionTimeoutSinceBeforeTimeout() {
        ...
        when(everaHubService.getChainTask(task.getChainTaskId())).thenReturn(Optional.of(chainTask));
    ...
    public void shouldUpdateFromInitializedOrRunning2ContributionTimeout() {
        ...
        when(taskRepository.findByChainTaskId(CHAIN_TASK_ID)).thenReturn(Optional.of(task));
        when(everaHubService.getChainTask(CHAIN_TASK_ID)).thenReturn(Optional.of(chainTask));
    ...
    public void shouldUpdateRunning2ConsensusReached() {
        ...
        when(everaHubService.getChainTask(task.getChainTaskId())).thenReturn(Optional.of(ChainTask.builder()
                .status(ChainTaskStatus.REVEALING)
                .winnerCounter(2)
                .build()));
        when(taskRepository.findByChainTaskId(CHAIN_TASK_ID)).thenReturn(Optional.of(task));
    ...
    public void shouldNotUpdateRunning2ConsensusReachedSinceCannotGetChainTask() {
        ...
        when(everaHubService.getChainTask(task.getChainTaskId())).thenReturn(Optional.empty());
    ...
    public void shouldUpdateResultUploading2Uploaded2Finalizing2Finalized() { //one worker uploaded
        ...
        when(everaHubService.canFinalize(task.getChainTaskId())).thenReturn(true);
        when(everaHubService.getChainTask(any())).thenReturn(Optional.of(chainTask));
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(everaHubService.finalizeTask(any(), any())).thenReturn(Optional.of(new ChainReceipt()));
    ...
    public void shouldUpdateResultUploading2Uploaded2Finalizing2FinalizeFail() { //one worker uploaded && finalize FAIL
        ...
        when(everaHubService.getChainTask(any())).thenReturn(Optional.of(chainTask));
        when(everaHubService.hasEnoughGas()).thenReturn(true);
        when(everaHubService.finalizeTask(any(), any())).thenReturn(Optional.empty());
    ...
    public void shouldWaitUpdateReplicateStatusFromUnsetToContributed() {
        ...
        when(everaHubService.getChainContribution("chainTaskId", "0x1")).thenReturn(Optional.of(chainContribution));
